...
def sceneDist(p):
    sphere1 = sphereDist(p, [-1.5, 1.3, 0], 1.3)
    
    sphere2 = sphereDist(p, [1.5, 0, 0], 1.0)
    plane = p[1]
    
    return min(min(sphere1, plane), sphere2)
...
while True:
    ...
    for x in range(res[0]):
        setPos(x*scale-res[0]*scale/2, res[1]/2)
        for y in range(res[1]):
            color = shader([x/res[0], y/res[1]], t)
            setPenColor(color)
            forward(scale)
